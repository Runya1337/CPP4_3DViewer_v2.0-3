...
namespace s21 {
    ...
    Event::VerifyExecution FindMinMax::Execute(s21::DataModel *data, Command command) {
        auto v = data->GetAllVertex().begin();
        InitMinMax(*v);
        if (command == Event::Command::kFindMinMax) {
            for (auto &vertex : data->GetAllVertex()) {
                if (min_.x >= vertex.x)
                    min_.x = vertex.x;
    ...
    void FindMinMax::InitMinMax(DataModel::CoordinatePoint &vertex) {
        min_.x = vertex.x;
        min_.y = vertex.y;
        min_.z = vertex.z;
        max_.x = vertex.x;
    ...
    Event::VerifyExecution FindMax::Execute(s21::DataModel *data, Command command) {
        data->GetAllVertex();
    ...
    Event::VerifyExecution FindCentre::Execute(s21::DataModel *data,
                                            Command command) {
